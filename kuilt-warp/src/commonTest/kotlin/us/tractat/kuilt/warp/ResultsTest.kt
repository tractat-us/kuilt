...
class ResultsTest {
    ...
    fun emptyResultsHasNoEntries() {
        assertTrue(Results.empty().taskIds.isEmpty())
    }

    @Test
    fun recordedResultIsRetrievable() {
        val r = Results.empty().record(alice, "task-1", timestamp = 1L, result = "done")
        assertEquals("done", r["task-1"])
    }

    @Test
    fun absentTaskReturnsNull() {
        assertNull(Results.empty()["task-99"])
    }

    @Test
    fun duplicateExecutionSameTaskIsAbsorbedIdempotently() {
        // Alice finishes task-1 at t=1; bob also finished it at t=2 (later → wins)
        val ra = Results.empty().record(alice, "task-1", timestamp = 1L, result = "alice-result")
        val rb = Results.empty().record(bob, "task-1", timestamp = 2L, result = "bob-result")
    ...
    fun duplicateExecutionSameTimestampBreaksTieOnReplicaId() {
        ...
        val ra = Results.empty().record(alice, "task-1", timestamp = 5L, result = "alice-result")
        val rb = Results.empty().record(bob, "task-1", timestamp = 5L, result = "bob-result")
        val merged = ra.merge(rb)
    ...
    fun mergeIsCommutative() {
        val ra = Results.empty().record(alice, "task-1", timestamp = 3L, result = "alpha")
        val rb = Results.empty().record(bob, "task-1", timestamp = 7L, result = "beta")
        assertEquals(ra.merge(rb), rb.merge(ra))
    }

    @Test
    fun mergeIsIdempotent() {
        val r = Results.empty().record(alice, "task-1", timestamp = 1L, result = "result")
        assertEquals(r, r.merge(r))
    }

    @Test
    fun distinctTasksFromDifferentReplicasAllSurviveMerge() {
        val ra = Results.empty().record(alice, "task-A", timestamp = 1L, result = "A-done")
        val rb = Results.empty().record(bob, "task-B", timestamp = 1L, result = "B-done")
        val merged = ra.merge(rb)
    ...
    fun earlierResultIsOverwrittenByLater() {
        ...
        val r1 = Results.empty().record(alice, "task-1", timestamp = 1L, result = "v1")
        val r2 = Results.empty().record(alice, "task-1", timestamp = 2L, result = "v2")
    ...
    fun threeWayMergeConvergesOnHighestTimestamp() {
        ...
        val ra = Results.empty().record(alice, "task-1", timestamp = 1L, result = "a")
        val rb = Results.empty().record(bob, "task-1", timestamp = 3L, result = "b")
        val rc = Results.empty().record(carol, "task-1", timestamp = 2L, result = "c")
