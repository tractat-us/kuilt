...
internal class RgaConformanceTest : QuiltedConformanceSuite<Rga<String>>() {
    ...
    private val base = Rga.empty<String>()
     ...
     * same reading `RgaConvergenceTest`'s `remove-head` op takes.
     */
    private val yOnly = xy.removeAt(0)!!.first

    /**
     * "x" comes back under a **fresh** [us.tractat.kuilt.crdt.RgaId] — the tombstone is on the old
    ...
    override fun samples(): List<Rga<String>> = listOf(base, x, xy, yOnly, xz, xReAsserted)

    override val retirementIsMeaningful: Boolean get() = true

    override fun retirementReAssertion(): RetirementReAssertion<Rga<String>> =
