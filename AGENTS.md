...
| `:kuilt-raft` | all | Raft consensus over a `Seam` — leader election + PreVote, log replication, log compaction + chunked `InstallSnapshot`, dynamic membership, linearizable reads (`readIndex()`, §3.6/§3.7) and graceful leadership transfer (`transferLeadership()` via TimeoutNow, §3.10). Ships the reference durable adapter `DurableStoreRaftStorage` (term/vote/leader-pin, log and snapshot as three fixed keys, one store per node), so it depends on `:kuilt-store` as well as `:kuilt-core`. |
...
`:kuilt-crdt`, `:kuilt-raft` → `:kuilt-store`). The dependency arrow never
points back into `:kuilt-core` — it must stay free of fabric-specific imports.
...
| Static analysis | there is none — see below |

**kuilt has no Kotlin linter (#2540).** No detekt or ktlint. Vale checks documentation prose only (see Readability checks below). `detektAll` and `detekt` are not tasks; a brief or a doc telling you to run either is stale. Static analysis here is **32 hand-rolled `check`-wired Gradle tasks** in the root `build.gradle.kts` (counted as distinct `verify*` / `forbid*` dependencies in the final `allprojects` block) — including `verifyDocCitations`, `verifyModuleTable`, `verifySampleLinks`, `verifySamplesAreRun`, `verifySeamHarnessCoverage`, `forbidTightRunTestTimeout`, `forbidProductionDispatcherInTests`, `forbidNotNullAssertionInUnresolvedSource`, and `forbidBareRunCatching`. They are source scans, so unlike a linter's per-source-set task graph they cover **every** source set on **every** platform, `:spike` and `commonTest` included. `./gradlew check` runs all of them; each has a long header comment saying exactly what it does and does not see.

**Why there is no Kotlin linter, so nobody re-derives it.** detekt ran here until #2540 and enforced nothing that was both working and unique. `buildUponDefaultConfig = false` meant only five rules were live. The four production ones all need **type resolution**, and detekt 1.23.8 pinned a Kotlin 2.0.21 frontend whose metadata **read ceiling** is `[2,1,0]` while every binary on the classpath is `[2,2,0]`–`[2,4,0]`; metadata a frontend cannot read is **silence, not an error**, so those rules saw only compiler built-ins, Java/JDK types, and declarations in the task's own source files (#2471). `val m: MutableMap<Long, String> = mutableMapOf(); m.remove(k)!!` was REPORTED while `val m = mutableMapOf<Long, String>(); m.remove(k)!!` was SILENT — same file, same task — and repo-wide `detektAll` was green on 12 true-positive `!!`. The fifth rule, `ForbiddenImport` on `Dispatchers`/`GlobalScope` in test sources, was a strict **subset** of `forbidProductionDispatcherInTests`: proved by experiment on four planted probes before removal — both import shapes in `jvmTest` red on both; the same import in `commonTest` and a member import `Dispatchers.Default` in `jvmTest` red on the guard and **blind** to detekt. What dropping it did cost: the built-in- and Java-typed sliver of `UnnecessaryNotNullOperator` and `CastNullableToNonNullableType`, the two rules with no lexical substitute — accepted deliberately. Re-adopting a linter is not foreclosed (detekt 2.x under `dev.detekt` pins this repo's exact Kotlin and would restore type resolution in full), but a proposal has to beat that measurement, not the idea of linting.

**Android Lint is the same shape one layer over, and it sits *inside* `ci-required` (#2595).** `lint` runs from `check` → `build` → `ci-required` with AGP's default `abortOnError = true`, across every `kuilt.kmp-library` module × three analyze tasks — and it is **blind to every Kotlin *library* symbol**. Lint 31.13.2 (AGP 8.13.2) bundles `com.android.tools.external.com-intellij:kotlin-compiler`, whose `META-INF/compiler.version` reads `2.2.20-Beta1-for-lint`, so its read ceiling is `[2,3,0]` while kotlin-stdlib/kotlin-test 2.4.10, play-services-nearby and every sibling `us.tractat.kuilt_*.kotlin_module` are all `[2,4,0]`. Unreadable metadata is **silence**, so lint prints "No issues found" in both states. Silently unenforced: every check keyed on a Kotlin library symbol — stdlib, coroutines, serialization, Ktor, any sibling kuilt module. Still live: everything keyed on **Java/JDK/Android** APIs, which is what the positive controls fired on (four Java/Android arms red, both kotlin-stdlib arms silent, and forcing `kotlin-stdlib:2.2.20` onto one module made the silent arms fire — causation, not correlation). The **Kotlin compile is fine** — `androidUnitTest` resolves a full classpath; the emitter is lint alone. **There is nothing to bump**: lint's frontend is pinned by AGP, the same trade detekt's was before #2540 dropped it. So the decision, recorded rather than left implicit, is that **lint stays in `check`** — a partial gate beats none for a library with this thin an Android surface — and `forbidLintFrontendSkew` in the root build reds when AGP's bundled frontend catches up, so this paragraph cannot quietly become false. What was **not** measured: whether any *real* finding is missed repo-wide. Treat a green `lint` as evidence about Java/Android APIs only.

**`!!` is banned by `forbidNotNullAssertionInUnresolvedSource`**, a per-file count ratchet over every source set under a module's `src/` (887 Kotlin sources in the task's output, #2039/#2471), and it is now the only thing in the repo that sees the operator. It sees neither a typealias nor a helper. `commonTest`/`commonSamples`/`jvmAndAndroidTest` are the one carve-out, too big to baseline today (#1960).

**Verify cache-disabled before auto-merge: `./gradlew clean && ./gradlew :<module>:build --rerun-tasks`.** Four false greens recur here. (1) `jvmTest` (or a scoped `:module:jvmTest`) does **not** compile the Android variant — a `commonTest` source can compile on JVM yet fail `compileDebugUnitTestKotlinAndroid` (and Kotlin/Native test targets) on a type-inference difference the JVM compiler accepts. CI runs the full `./gradlew build`, so it catches this; your local `jvmTest` won't. (2) Gradle's **build cache** can serve a stale `FROM-CACHE` "success" for a test-compile task whose source is actually broken, so a re-run "passes locally" without executing the failing code. Before enabling auto-merge on a code PR, run the **full module build** with `--rerun-tasks` (add `--no-build-cache` if any test-compile task still shows `FROM-CACHE`) and confirm the tasks are genuinely `EXECUTED`. "Built locally" via `jvmTest` or a cached build is not proof the Android/Native variants compile. (3) A **`:<module>:build`-scoped build is a false green for consensus/runtime *behavior* changes** — even the full *module* build (not just `jvmTest`) skips the downstream `:examples`/`:kuilt-cluster` **E2E cluster tests**, which exercise the whole runtime stack. A change to `:kuilt-raft` consensus *behavior* (election / replication / membership / forwarding) that passes every `:kuilt-raft` test can still break a cluster E2E invariant — e.g. a forward-reaping change broke `ClusterClientMultiClientHardeningE2ETest`'s "no double-apply", entirely invisible to `:kuilt-raft:build`. For any consensus-*behavior* change, run the **full `./gradlew build`** (or at minimum add `:examples:test`), not a module-scoped build. (4) **`--rerun-tasks` alone is not "cache-disabled": it does not clear Kotlin's incremental-compilation state, which is why `clean` leads the command (#1913).** That state is not a tracked task *output*, so re-executing a task need not delete it. Measured on Kotlin 2.4.20 with Kotlin/Wasm incremental compilation on (#1914) by running one `:kuilt-core:build --rerun-tasks` after planting a sentinel file in every `build/kotlin/<task>/` IC directory: the IC directories of ordinary **compile** tasks *are* cleared (**0 of 9** tasks kept a sentinel, 0 of 24 sentinels — Gradle's stale-output cleanup reaches them), while the Kotlin/Wasm **link** task `compileTestDevelopmentExecutableKotlinWasmJs` keeps **all** of its state: both of its `build/kotlin/` directories, and its whole cache under `build/klib/cache/` — **19 of 19** directories and **1,908 of 1,908** files unchanged, the standard library's 865 included — with that task confirmed `> Task` `EXECUTED` in the same run, so this is the task family and not a skipped task. The re-run rewrote none of those files; it *read* them. A task therefore re-runs "from scratch" while still reading state an earlier, differently-shaped execution wrote — the condition that produced #1893's ICE. (With wasm IC off, as it was until #1914, the same task kept one of its two `build/kotlin/` directories and had no `klib/cache` at all.) That cache is witnessed by inode and modification time rather than a sentinel, because a file planted in its `version.<hash>/` directory is itself an IC internal error. **`clean` is the mechanism because it is a *task*, and only a task runs on a configuration-cache hit.** With `org.gradle.configuration-cache=true` (the default here) a hook in `settings.gradle.kts` on `gradle.startParameter.isRerunTasks` — the fix #1913 originally proposed — **never runs**: `--rerun-tasks` *reuses* the configuration-cache entry, so settings is not evaluated at all, and a `-P` gate flag fares no better, firing once on the invocation that changes it and silently no-oping on every one after. Both were measured with a `println` in `settings.gradle.kts`; it prints only under `--no-configuration-cache`. **Do not "optimise" this into a wipe keyed to specific paths** — the paths move, and have already: #1913's own table named `<module>/build/klib/cache` (37 dirs when filed), which went to **0** while `kotlin.incremental.wasm=false` was set and came back when #1914 removed it, while the `build/kotlin/<task>/` path that table recorded as empty kept state throughout. `clean` is backend-agnostic and cannot rot. That matters because a new retaining family is one property away. Kotlin/Native link IC (`kotlin.incremental.native`) was on by default in 2.4.20-Beta2 and is off again in 2.4.20; opted in on 2.4.20, its `build/kotlin-native-ic-cache/` kept **3 of 3** sentinels through `--rerun-tasks` and lost them to `clean`. Two caveats: root `clean` does not reach the `build-logic` **included build**, whose two IC directories belong to ordinary compile tasks and so are covered by the mechanism above; and `clean` is a separate invocation (`&&`) rather than a task in the same one because `org.gradle.parallel=true` makes intra-invocation `clean`-versus-build ordering a thing to reason about rather than rely on.
...
**And do not then treat one target as the representative — the coverage is the UNION (#1979).** Once you know JVM is blind to a determinism defect, the tempting economy is to run a native target *instead*. That is wrong in the other direction: measured on #1979 the two families reddened **disjoint arms of the same suite** — JVM caught the three *associativity* arms and passed `convergesAtSeedZero`, while `macosArm64` and `wasmJs` caught *permutation* and *commutativity* and passed all three associativity arms. Same seeds, same code; `HashMap` is bucket-ordered on JVM and insertion-ordered on Native/wasm, so each walks a different trajectory to the same defect. Run all three and read the rows together. Two corollaries that are easy to get backwards: a differing failure **count** across targets (5 vs 3) is usually different **arms**, not different severity — compare which assertions fired, never the total; and a **`failures=0` row taken PRE-fix is the vacuous case, not a pass** — that arm cannot see the defect at all, so its green afterwards proves nothing and is an evidence-grounds block whatever CI says.
...
- **Build logic is centralized** in `build-logic/`: `kuilt.kmp-library` defines
  ...
  does **not** — a bench harness, a KSP processor, a demo app — needs no extra
  lint wiring: there is no Kotlin linter, and the root build's guards are source scans
  that reach every module by construction (#2540 removed `kuilt.detekt-jvm`,
  `kuilt.detekt-kmp` and the `forbidUnlintedModule` guard that policed them).
  ...
  real extracting `:kuilt-store` (see #2498). Since #2770 there is a **fourth**
  edit, and it is mechanical: run `python3 .github/scripts/llms-txt.py` and commit
  the result, because that same POM description is what the generated `llms.txt`
  shows a coding agent in a consumer repo. CI reds with the command if you forget.
  The index derives its module list from exactly the branches of
  `moduleDescription()`, so that missing fallback is also what keeps the index
...
- **Spec-critical refactors get a spec-conformance review, not just a behavior-preservation
  review** — see `.claude/rules/consensus.md`.
...
- **Coroutine test determinism:** types that own a `CoroutineScope` take an
  ...
  - **Production dispatchers are banned in test sources** (`Dispatchers.{Unconfined,Default,IO,Main}`,
    ...
    reason is mandatory. **An `@Suppress("…")` is not a mechanism here** — those name detekt rules,
    and the repo has no detekt (#2540). Even when it did, `ForbiddenMethodCall` was configured
    nowhere and never fired (#1329, 21 such annotations swept by #1934), and
    `@file:Suppress("ForbiddenImport")` named a rule that only ever reached `jvmTest` and the two
    `androidUnitTest` source sets. The marker is the only mechanism.
...
Two published surfaces plus a generated index, deployed to GitHub Pages by `.github/workflows/docs.yml` on every push to `main`:
...
**`llms.txt` is generated — never hand-edit it (#2770).** The root `llms.txt` is the index a
coding agent in a *consumer* repo reads to find these surfaces, and it exists twice on purpose,
...
it reads four files, needs no toolchain, and runs before the JDK setup in that job. Its one
assumption the tree cannot settle — that the API site still holds a page per module at
`api/<module>/index.html` — is checked by `--verify-site` against the assembled site, in the deploy,
before the upload.
