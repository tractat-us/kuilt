...
**kuilt has no Kotlin linter (#2540).** No detekt or ktlint. Vale checks documentation prose only (see Readability checks below). `detektAll` and `detekt` are not tasks; a brief or a doc telling you to run either is stale. Static analysis here is **nine hand-rolled `check`-wired Gradle tasks** in the root `build.gradle.kts` — `verifyDocCitations`, `verifyModuleTable`, `verifySampleLinks`, `verifySamplesAreRun`, `verifySeamHarnessCoverage`, `forbidTightRunTestTimeout`, `forbidProductionDispatcherInTests`, `forbidNotNullAssertionInUnresolvedSource`, `forbidBareRunCatching`, and a dozen more siblings. They are source scans, so unlike a linter's per-source-set task graph they cover **every** source set on **every** platform, `:spike` and `commonTest` included. `./gradlew check` runs all of them; each has a long header comment saying exactly what it does and does not see.
...
**`!!` is banned by `forbidNotNullAssertionInUnresolvedSource`**, a per-file count ratchet over every source set under a module's `src/` (833 files, #2039/#2471), and it is now the only thing in the repo that sees the operator. It sees neither a typealias nor a helper. `commonTest`/`commonSamples`/`jvmAndAndroidTest` are the one carve-out, too big to baseline today (#1960).
...
`--no-exit` to enforce them. Entry pages target reading grade 6,
