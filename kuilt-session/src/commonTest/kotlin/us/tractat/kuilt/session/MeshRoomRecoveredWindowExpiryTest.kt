...
class MeshRoomRecoveredWindowExpiryTest {
     ...
     * declares the *survivor* partitioned at t=2600. The split does not heal — it spreads. That
     * half is asserted here as its own arm — see the `Liveness.Connected` assertion below, which the
     * `holds` assertions above **cannot** cover: the member still holds the survivor in its roster
     * the whole time, just as `Partitioned`, so a membership-only check is green while the split
     * spreads underneath it.
     */
    @Test
    fun `a member that recovered inside its window is still seated everywhere after the window elapses`() =
        runTest(StandardTestDispatcher(), timeout = TEST_WEDGE_BACKSTOP) {
            ...
            assertAll(
                ...
                {
                    // The SPREADING half, and the one arm `holds` above structurally cannot see.
                    // `handleFarewell` calls `stopDetector(departed)`, so a survivor that evicted
                    // the recovered member stops answering its pings and the member declares the
                    // *survivor* partitioned one timeout later — while still holding it in the
                    // roster. Membership stays intact on this edge and only the liveness moves, so
                    // a `holds` check is green throughout. On `main` this reads
                    // `Partitioned(until=4600)`; the split is a second edge, not one.
                    assertEquals(
                        Liveness.Connected,
                        mesh.dropped.roster.value.firstOrNull { it.id == survivorId }?.liveness,
                        "the recovered member must not go on to declare the survivor partitioned; " +
                            "handleFarewell's stopDetector is what spreads the split — " +
                            "${mesh.dropped.rosterLine("dropped")}",
                    )
                },
                {
