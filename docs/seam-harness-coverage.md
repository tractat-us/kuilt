...
| `InMemorySeam` | `kuilt-core/src/commonMain/kotlin/us/tractat/kuilt/core/InMemoryLoom.kt` | `InMemoryLoomConformanceTest` | `InMemoryLoom.newSeam` mints it and the harness returns `loom to loom`; also the joiner tier of `TieredSeamConformanceTest`. It was the base of `GossipSeamConformanceTest` until #2605 swapped that harness onto `peerMesh`, so nothing in the gossip module constructs one any more. |
