...
| `ResumableChannel` | `kuilt-core/src/commonMain/kotlin/us/tractat/kuilt/core/MuxClientLoom.kt` | none | Minted only by `MuxClientLoom.weave`. The one in-tree factory a harness could reach it through is `InMemoryRoomFabric.clientLoom`, and the sole harness on that fabric takes `clientSeam` instead — so a reconnect-repointing client channel handle ships with no suite behind it. Bindable: it needs a `MuxClientLoom` harness, not a new abstraction. |
...
| `MCSessionLink` | `kuilt-multipeer/src/appleMain/kotlin/us/tractat/kuilt/multipeer/internal/MCSessionLink.kt` | none | The Apple `MultipeerConnectivity` link. `MultipeerConformanceTest` is a `jvmTest`, so it resolves the `jvmMain` actual and never touches this one; `kuilt-multipeer/src/appleTest` has a discovery-source suite but no `SeamConformanceSuite` subclass. The real Apple transport is unharnessed. |
