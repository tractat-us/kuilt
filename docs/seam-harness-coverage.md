...
position until #2441 bound it, and the binding immediately turned up a real defect (#2443, since
fixed), which is the argument for doing this rather than writing a reason.

## Registry

| Seam | Declared in | Harness | Why |
|------|-------------|---------|-----|
| `ChannelView` | `kuilt-core/src/commonMain/kotlin/us/tractat/kuilt/core/MuxBase.kt` | `MuxClientLoomConformanceTest`, `MuxServerLoomConformanceTest` | Both ends of `MuxClientLoomPair`: each `LazyMuxLoom.weave` goes through `MuxClientLoom.weave`, which mints this view under a `ResumableChannel`. Also the joiner side of the hub harness, where `RoomHubLoomPair.clientLoom.weave` returns `NamedMux(base, scope).channel(name)`. **The position is the point** (#2372): the hub harness had it as the *joiner* only, and every core close obligation closes the *host*, so `close()` was never called on a channel view and then looked at — the view reported the base's `Woven` forever while this row read as covered. |
...
| `InMemorySeam` | `kuilt-core/src/commonMain/kotlin/us/tractat/kuilt/core/InMemoryLoom.kt` | `InMemoryLoomConformanceTest` | `InMemoryLoom.newSeam` mints it and the harness returns `loom to loom`; also the joiner tier of `TieredSeamConformanceTest`, and the base under both ends of `MuxClientLoomConformanceTest`. It was the base of `GossipSeamConformanceTest` until #2605 swapped that harness onto `peerMesh`, so nothing in the gossip module constructs one any more. |
...
| `Mesh` | `kuilt-core/src/commonMain/kotlin/us/tractat/kuilt/core/fabric/MeshSeam.kt` | `PeerMeshConformanceTest`, `GossipSeamConformanceTest` | The interface `peerMesh(...)` returns and `MeshSeam` implements; `PeerMeshLoom.weave` calls `peerMesh(...)`, and since #2605 so does the gossip harness's `GossipLoom.weave` — one per end of a `connectionPair`, wrapped in a `GossipSeam`. |
| `MeshSeam` | `kuilt-core/src/commonMain/kotlin/us/tractat/kuilt/core/fabric/MeshSeam.kt` | `PeerMeshConformanceTest`, `GossipSeamConformanceTest` | `buildMesh` mints it behind `peerMesh(...)`, which the harness's `PeerMeshLoom.weave` calls — and, since #2605, `GossipLoom.weave`, so it is the base whose roster the gossip harness's joiner obligation now reads through. It is also the client base under `MuxServerLoomConformanceTest`'s channel view. |
| `ResumableChannel` | `kuilt-core/src/commonMain/kotlin/us/tractat/kuilt/core/MuxClientLoom.kt` | `MuxClientLoomConformanceTest` | Minted only by `MuxClientLoom.weave`; `MuxClientLoomPair` returns two such looms over one shared `InMemoryLoom` base, so this handle is both ends of the pair. The harness was written for #2441 and left unlanded because it **red on ungated core** — `close()` delegates to a `NamedMux` channel view whose `state` kept reporting the live base's `Woven`, failing `closeDrivesStateTornNormal`, and no capability flag may excuse an ungated obligation. #2372 fixed the view; re-running the harness against the pre-fix `MuxBase` still reds 5 of its 31 rows, all naming the same `actual Woven`. |
...
| `DelayedWovenSeam` | `kuilt-conformance/src/commonMain/kotlin/us/tractat/kuilt/conformance/DelayedWovenLoom.kt` | none | The one entry where `none` understates things: `SeamConformanceSuite.sendWhileWeavingDoesNotThrow` builds a `DelayedWovenLoom` inline, so every harness drives this seam through that one obligation. No harness returns it from `newLoomPair`, so the rest of the suite never touches it. Its close-once guard is pinned separately by the real-threaded `DelayedWovenSeamCloseOnceConcurrencyTest` (#2328), which is not a `newLoomPair` binding and so cannot appear in the harness column. |
...
| `GossipSeam` | `kuilt-gossip/src/commonMain/kotlin/us/tractat/kuilt/gossip/GossipSeam.kt` | `GossipSeamConformanceTest` | The harness returns two `GossipLoom`s, one per end of a single `connectionPair`; each `weave` builds a `peerMesh` over its own end and wraps it in a started `GossipSeam`. It used to return one loom over one `InMemoryLoom` twice — swapped in #2605 so each role owns its roster and the suite's joiner obligation can actually fail here (`joinerRosterOrigin` is now `TheJoinPath`). |
