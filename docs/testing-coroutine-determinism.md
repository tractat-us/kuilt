...
The one place this `MUST` inverts is **inside a `withContext(NonCancellable)` shield** — a farewell broadcast on a teardown path, say. There your own job is never cancelled, so every `CancellationException` reachable inside the shield is callee-minted and rethrowing it skips the rest of the cleanup; use a plain `try` / `catch (failure: Throwable)` + debug log instead. `forbidRunCatchingCancellableUnderNonCancellable` fails the build on the `runCatchingCancellable` form there. See CLAUDE.md's exception-discipline rule and #1803/#1824.
