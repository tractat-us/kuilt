...
| `meshSeam()` → `Mesh` | **Connections → Seam** | *Topology builder.* N point-to-point links woven into one fully-connected N-peer `Seam`; learns each remote id via a mesh preamble (id + per-connection nonce) and dedups duplicate links from a simultaneous dial by a canonical, order-independent link nonce both ends agree on. A deduplicated loser is **drained, not dropped**: it is read until the remote's in-band goodbye, and the surviving link's frames are held behind that tail, so a simultaneous dial costs no frames and no reordering. Admits later joiners via `Mesh.addLink`. |
