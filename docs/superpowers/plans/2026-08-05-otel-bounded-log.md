...
| 2 | Bytes on disk are O(`maxRecords`·record + replicas), not Θ(records ever) | **Met on the export path** — `bytesOnDiskAreBoundedByTheWindowNotByRecordsEverExported`, with a Θ(records) control arm. Not flat to the byte: the residual creep is CBOR integer width and it saturates (2,444 → 2,518 → 2,524 → 2,524 at 100/1k/10k/50k records), so the assertion is on the shape |
