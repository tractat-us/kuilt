...
### 4.6 Two REQUIRED wiring invariants (NEW in v3 — the §4 the review flagged)

The §4 predicate above specifies *what* `retainedFrontier` must hold but not the
*ordering* and *concurrency* discipline its mutation requires. Both are part of the
correctness argument, not implementation taste. **Violating either re-opens the
#275 safety hole even though the logical predicate is unchanged.** They constrain
the replicator wiring (#268–270); they are stated here because they belong to the
*design*, not to any one PR.

- **(W1) Retain-capture-before-drop atomicity.** In `evictStalePeers`, folding the
  evicted peer's row into `retainedFrontier` (`ceilWith(frontiers[c])`, §4.3) and
  removing its live row from `frontiers` must be a **single atomic state
  transition** — there must be **no observable intermediate state** in which the
  evicted row is gone from `frontiers` (so `F_live` has already dropped) but its
  dots have not yet entered `retainedFrontier` (so `F` has not yet floored). A
  `compact` query that observed that intermediate would compute `F` below a
  known-to-exist dot and could purge `I` — exactly #275. The retain and the drop
  are two halves of one operation; apply them together (compute the new
...
- **(W2) Single-coroutine confinement of matrix-clock mutations.** `SeamReplicator`
  ...
  **only because every mutation to them is confined to one coroutine context**. The
  retained-frontier bookkeeping (retain on eviction, normalise on every matrix
  update, §4.4) **must not** be moved into a separately-launched coroutine, and
  `scope` **must not** be backed by a multithreaded dispatcher. Without this, (W1)'s
  "single atomic transition" is not even well-defined: a concurrent reader on
  another thread could interleave between the retain and the drop with no happens-
  before edge. (The `staleOrdering` adversarial probe in #277 pins the ordering this
  invariant protects.)
