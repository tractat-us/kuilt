...
**Primitive:** `Room.incomingFrom(member)` (`us.tractat.kuilt.session`). A room admits and routes before a `host { onRoom }` consumer runs, so a per-member collector of `incoming` subscribes after that member's first frame by construction, and the frame is gone (#2802). `incomingFrom` holds the member's frames from the instant it is admitted and is per admission: the flow completes when the member leaves, and a member admitted again gets a fresh inbox. A claim for a peer with no current admission completes at once, exactly as an admission that has just ended does. It is never silently lossy — when frames cannot be held (the inbox overflowed before anyone claimed it, or the collector fell behind) the flow fails with `FramesLost`, carrying how many frames went and which of the two it was. A consumer that never reads per-member frames builds its rooms with `SeamRoomFactory(memberInboxCapacity = 0)` and holds nothing. **That failure is terminal for the admission**: every later read of it fails the same way and nothing restarts the stream, so treat the member as lost — evict it, or let its session drop and re-join, which is a new admission. Collect it from one coroutine at a time; a second concurrent collection throws, including one started straight after `cancel()` — `cancelAndJoin` the previous collection first. A collection that is cancelled leaves every frame its collector had not received for the next one; a frame your own pipeline did receive is yours, at-most-once as for any Flow. That includes any operator that takes frames ahead of you — `buffer`, `flowOn`, `conflate`, `produceIn`, `shareIn` / `stateIn` — which loses what it holds when cancelled, so the guarantee stops at the first one.
