...
| `TieredSeam` (`:kuilt-core`) | Inherits, per tier | Inherits — **and drops silently** | [`TieredSeam.kt:180`](../kuilt-core/src/commonMain/kotlin/us/tractat/kuilt/core/TieredSeam.kt#L180) | Routes to whichever tier owns the id; an id owned by **neither** tier is discarded with no `PeerNotConnected`. The worst variant for #1576: even a caller that handles the exception sees nothing. |
