...
- a reconnect banner, your own connected/retrying/failed enum, "why did the session drop?" → `MembershipEvent.Partitioned.reason` (`ReconnectReason`) + `MembershipEvent.HostLost.reason` (`FailureReason`)
